...
class MyPreference @Inject constructor(private var mSharedPref: SharedPreferences?) {
    ...
    fun saveResponseCount(context: Context, count: Int) {
        val sharedPreferences = context.getSharedPreferences("chat_prefs", Context.MODE_PRIVATE)
        val editor = sharedPreferences.edit()
        editor.putInt("response_count", count)
        editor.apply()
    }

    fun loadResponseCount(context: Context): Int {
        val sharedPreferences = context.getSharedPreferences("chat_prefs", Context.MODE_PRIVATE)
        return sharedPreferences.getInt("response_count", 10) // Default to 10 if not found
    }
